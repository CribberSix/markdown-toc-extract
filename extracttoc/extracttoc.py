...
def format_header(header): 
	""" Removes leading or trailing whitespaces. Calculates the level of the header. Creates the markdown-link. 
...
def remove_code_blocks(content):
	...
	Since code blocks can contain lines with leading hashtags (e.g. comments in python) they need to be removed before looking for headers. 
...
def create_toc(toc_levels):
	...
	:param toc_levels: A list containing triples consisting of the header, the level of the header and a formatted markdown-link to the header. 
	:type toc_levels: List<Triple<str, int, str>>
...
def main(): 
	...
	parser = argparse.ArgumentParser(prog ='extracttoc',
										description ='Extracts the table of contents from a markdown file.')

	#parser.add_argument('markdownfile', metavar='f', type='string', help='the markdownfile from which to extract a toc.')
